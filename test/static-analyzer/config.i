...
_Bool 
# 32 "src/config.h"
    readConfigFile(char *configFileName, Config *outConfig);
void printConfig(Config config);
# 2 "src/config.c" 2
# 1 "src/config_internal.h" 1
...
# 25 "src/config_internal.h" 3 4
_Bool 
# 25 "src/config_internal.h"
    tokenizeConfig(Buffer *buff, ConfigTokenList *outList);
# 3 "src/config.c" 2
...
# 28 "src/config.c"
        ...
        else if (peek(buff) == ':') { appendConfigTok(outList, (ConfigToken){ ':' }); consume(buff);}
        else if (peek(buff) == ',') { appendConfigTok(outList, (ConfigToken){ ',' }); consume(buff);}
        else if (peek(buff) == '[') { appendConfigTok(outList, (ConfigToken){ '[' }); consume(buff);}
        else if (peek(buff) == ']') { appendConfigTok(outList, (ConfigToken){ ']' }); consume(buff);}
...
void printConfigTokens(ConfigTokenList tokens) {
    ...
    for (size_t i = 0; i < tokens.numTokens; i++) {
        if (tokens.tokens[i].type == ConfigToken_String) {
            ...
            vlog("src/config.c", 71, Severity_Fatal, "Config: Found invalid config token when printing: %d\n", tokens.tokens[i].type);
        }
    }
}
...
_Bool 
# 76 "src/config.c"
    parseConfigValue(ConfigTokenList *tokens, ConfigValue *outValue) {
...
# 86 "src/config.c"
            ...
            if (tokens->tokens[tokens->curToken].type == ',') {
                ...
                vlog("src/config.c", 100, Severity_Error, "Config: Invalid token between list elems: %c\n", tokens->tokens[tokens->curToken].type);
                return 
# 101 "src/config.c" 3 4
                      0
# 101 "src/config.c"
                           ;
            }
...
# 143 "src/config.c"
        ,
                                                 144
# 143 "src/config.c"
        , Severity_Error, "Config: Invalid config token when parsing a value. Expected a string or '[': %d\n", tokens->tokens[tokens->curToken].type)
                                                  ;
        return 
# 145 "src/config.c" 3 4
              0
# 145 "src/config.c"
                   ;
    }
...
# 149 "src/config.c" 3 4
_Bool 
# 149 "src/config.c"
    readConfigFile(char *configFileName, Config *outConfig) {
    Buffer buffer = {0};
    if (!openAndReadFileToBuffer(configFileName, &buffer)) {
        char *fileError = strerror(
# 152 "src/config.c" 3 4
                                  (*__errno_location ())
# 152 "src/config.c"
                                       );
        vlog(
...
# 153 "src/config.c"
        ,
                                     154
# 153 "src/config.c"
        , Severity_Error, "Config: Couldn't read config file with name: %s. \nError msg: %s\n", configFileName, fileError)
                                      ;
        return 
# 155 "src/config.c" 3 4
              0
# 155 "src/config.c"
                   ;
    }
